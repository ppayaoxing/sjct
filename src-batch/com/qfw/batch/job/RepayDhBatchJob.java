 ...
 *
 */
public class RepayDhBatchJob extends BatchDependJob {
	...
	public String runJob() throws Exception {
		...
		//System.out.println("自动到期还款(代付)批处理开始执行,当前日期为:"+batchDate);
		IBatchBS batchBS = (IBatchBS)ApplicationContextUtil.getBean("batchBS");
		batchBS.autoRepayForFullDh(batchDate);
		//System.out.println("自动到期还款(代付)批处理结束执行");
		return "自动到期还款(代付)批处理执行成功";
	}
