...
<form class="form-horizontal" id="submitForm2" method="post" action="${base}/userFinancial/addAutoTenderConfigSubmit.do" autocomplete="off"  data-validator-option="{theme:'yellow_right_effect',stopOnError:true}" >
                <div id="myAutoTender_tab2" class="repayment" style="display:none">
                	...
                	/>
					<input type="hidden" id="isAuto" name="isAuto" value="1" />
                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="person_info">
                      ...
                      <tr>
                      	<td colspan="2">
                      		...
                      		<#if configBO.id??>
                      			<#if configBO.isAuto==1 && configBO.status==0>
                      				<button class="ui-button ui-button-blue ui-button-mid" id="openBtn">开户自动工具</button>
                      			<#else>
                      				<button class="ui-button ui-button-yellow ui-button-mid" id="cancelBtn">取消自动工具</button>
                      			</#if>
