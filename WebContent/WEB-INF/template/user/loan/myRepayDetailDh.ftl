...
//<![CDATA[
	...
	function ajaxQuery(requestPage,isInit){
		$.ajax({
				type:"POST",
				url:"ajaxQueryMyRepayDetailDh.do",
				dataType:"json",
				...
				success:function(data) {
			        ...
			        $.each(data.list, function(i, n){//REPAY_TYPE_CD
			        	...
			        	if(n.repay_status_cd!="已还"){
			        		row.append($("<td><a href='javascript:void(0);' class=\"Operation ui-button ui-button-blue ui-button-mid\"  onclick='return showPreRepayDiv("+loanId+"," + n.id + ");'>还款</a></td>"));
			        	}else{
